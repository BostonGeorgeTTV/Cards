...
ESX.RegisterUsableItem("micard_booster_pack1", function(source, item)
    ...
    TriggerClientEvent("bg_cardsclient:OpenBoosterPack1", source, item.name)
end)

ESX.RegisterUsableItem("micard_booster_pack2", function(source, item)
    exports.ox_inventory:RemoveItem(source, item.name, 1)
    TriggerClientEvent("bg_cardsclient:OpenBoosterPack2", source, item.name)
end)
-- Use Card
for _, name in ipairs({"micard_legendary", "micard_rare", "micard_basic"}) do
    ESX.RegisterUsableItem(name, function(source, item)
        ...
        TriggerClientEvent("bg_cardsclient:showCard", source, data)
    end)
end
...
-- mostra carte
RegisterNetEvent("bg_cardsserver:showCardToTarget")
AddEventHandler("bg_cardsserver:showCardToTarget", function(slot, cTarget)
